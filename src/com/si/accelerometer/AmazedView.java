...
public class AmazedView extends View {
	...
	// this is never used but just in-case future
	...
	/**
	 * Custom view constructor. * * @param context * Application context * @param
	 * activity * Activity controlling the view
	 */
	...
	/**
	 * * Called from gameTick(), update marble x,y based on latest values
	 * obtained * from the Accelerometer sensor. AccelX and accelY are values
	 * received from * the accelerometer, higher values represent the device
	 * tilted at a more * acute angle.
	 */
	...
	public void drawHUD() {
		...
		mCanvas.drawText(mStrings[TXT_TIME] + ": " + (mTotalTime / 1000),
				mTextPadding, mHudTextY, mPaint);
		mPaint.setTextAlign(Paint.Align.CENTER);
		mCanvas.drawText(mStrings[TXT_LEVEL] + ": " + mlevel, mCanvasHalfWidth,
				mHudTextY, mPaint);
		mPaint.setTextAlign(Paint.Align.RIGHT);
		mCanvas.drawText(mStrings[TXT_LIVES] + ": " + mMarble.getLives(),
				mCanvasWidth - mTextPadding, mHudTextY, mPaint);
		// do we need to display the warning message to save the user from
		// possibly dying instantly.
		if (mWarning) {
			mPaint.setColor(Color.BLUE);
			mCanvas.drawRect(0, mCanvasHalfHeight - 15, mCanvasWidth,
					mCanvasHalfHeight + 5, mPaint);
			mPaint.setColor(Color.WHITE);
			mPaint.setTextAlign(Paint.Align.CENTER);
			mCanvas.drawText(mStrings[TXT_TAP_SCREEN], mCanvasHalfWidth,
					mCanvasHalfHeight, mPaint);
		}
	}
	...
	public void drawGameOver() {
		mPaint.setColor(Color.BLACK);
		mPaint.setTextAlign(Paint.Align.CENTER);
		mCanvas.drawText(mStrings[TXT_GAME_OVER], mCanvasHalfWidth,
				mCanvasHalfHeight, mPaint);
		mCanvas.drawText(mStrings[TXT_TOTAL_TIME] + ": " + (mTotalTime / 1000)
				+ "s", mCanvasHalfWidth, mCanvasHalfHeight
				+ mPaint.getFontSpacing(), mPaint);
		mCanvas.drawText(mStrings[TXT_GAME_OVER_MSG_A] + " " + (mlevel - 1)
				+ " " + mStrings[TXT_GAME_OVER_MSG_B], mCanvasHalfWidth,
				mCanvasHalfHeight + (mPaint.getFontSpacing() * 2), mPaint);
	...
	public void drawGameComplete() {
		mPaint.setColor(Color.BLACK);
		mPaint.setTextAlign(Paint.Align.CENTER);
		mCanvas.drawText(mStrings[GAME_COMPLETE], mCanvasHalfWidth,
				mCanvasHalfHeight, mPaint);
		mCanvas.drawText(mStrings[TXT_TOTAL_TIME] + ": " + (mTotalTime / 1000)
				+ "s", mCanvasHalfWidth, mCanvasHalfHeight
				+ mPaint.getFontSpacing(), mPaint);
	...
	public void drawLandscapeMode() {
		...
		mCanvas.drawRect(0, 0, mCanvasWidth, mCanvasHeight, mPaint);
		mPaint.setColor(Color.BLACK);
		mCanvas.drawText(mStrings[TXT_LANDSCAPE_MODE], mCanvasHalfWidth,
				mCanvasHalfHeight, mPaint);
	}
	 ...
	 * required for changing game states could grow quickly. * * @param newState
	 * * New game state
	 */
	...
	/**
	 * * Unregister the accelerometer sensor otherwise it will continue to
	 * operate * and report values.
	 */
